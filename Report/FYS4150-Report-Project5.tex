...
\end{equation} 
This will however not be the case in our model for a finite value of $N$. EXPLAIN.

Eq. (\ref{eq:t_crunch}) can be derived ... DERIVATION!

Next we want to find $G$ in units of $\tau_{\mathrm{crunch}}$ so that $G$ becomes a function of the number of particles $N$ and the average mass of the particles $\mu$. DERIVATION.

After running calculations for the gravitational force as expressed in Eq. (\ref{eq:force_comp}), we will introduce a smoothing function that will take care of any numerical instability that arises when two of the particles come very close. We modify the Newtonian gravitational force as
...
\subsection{Testing the numerical methods}
...
\begin{tabular}{cc}
	...
	(a) Position $x$					& (b) Velocity $v_x$  \\[6pt]
\end{tabular}
...
The stability of the two methods described in Sec.~\ref{sec:methods} can be compared by running the two algorithms over a very long time period. 
