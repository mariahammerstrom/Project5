...
\end{equation}
where $n_0$ and $r_0$ are scaling constants. We follow Joyce \textit{et. al} (2010) and approximate these scaling constants with $r_0 \propto N^{-1/3}$ and $n_0 \propto N^2$. 

First we make histograms of the radial density in the equilibrium state, shown in Fig.~\ref{fig:Nbody_histogram}a-b. These histograms counts the occurrences of various radial positions of the star in the system, which are weighted by the volume of the sphere within each radii to get the number density.

The result is shown in Fig. XXXX. 
